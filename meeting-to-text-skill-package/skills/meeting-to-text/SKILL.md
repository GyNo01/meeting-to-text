...
- Video: `.mp4`, `.mkv`, `.mov`, `.avi`, `.webm`, `.m4v`, `.flv`, `.wmv`, `.mpg`, `.mpeg`, `.ts`, `.mts`, `.m2ts`, `.3gp`
- Audio: `.wav`, `.mp3`, `.m4a`, `.m4b`, `.aac`, `.flac`, `.ogg`, `.oga`, `.opus`, `.weba`, `.wma`, `.amr`, `.aiff`, `.aif`, `.caf`, `.mka`, `.ac3`
...
```powershell
& '<YOUR_CONDA_ENV_PYTHON_PATH>' 'C:\path\to\your\meeting-to-text\skills\meeting-to-text\scripts\meeting_to_text.py' --input '<SOURCE_PATH>' --output '<OUTPUT_TARGET>'
```
...
- `segment_count`: normalized diarization segments used for speaker assignment
- `transcription_plan_count`: full-audio timeline chunks sent into transcription
- `transcribed_segment_count`: chunks that produced text
- `skipped_segment_count`: chunks that failed or produced no text
...
The entrypoint enforces the workflow. Do not rewrite the pipeline ad hoc in the conversation.
...
- transcribe the full normalized audio timeline, then map each transcript chunk to the best matching speaker segment
...
- any warnings that matter for review
  ...
  转录完成后，必须调用 LLM 生成会议元信息，格式如下：
  【时间】：<根据音频内容推断或填写 "未识别">
  【参与人】：<从内容中识别并列出发言人，如 "说话人1、说话人2" 或根据声音特征推断>
  【主题/内容概要】：<用100字以内概括会议核心主题>
  将这段元信息 插入到转录文本的最开头，让用户一眼能看到会议概况。
