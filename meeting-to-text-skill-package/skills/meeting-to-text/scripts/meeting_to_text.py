...
MIN_DIARIZATION_SEGMENT_SECONDS = 0.40
MIN_TRANSCRIPTION_SEGMENT_SECONDS = 0.20
MERGE_GAP_SECONDS = 0.80
DIARIZATION_CHUNK_DURATION_SECONDS = 1.50
DIARIZATION_CHUNK_STEP_SECONDS = 0.75
MAX_TRANSCRIPTION_SEGMENT_SECONDS = 25.0
SAMPLE_RATE = 16000
...
@dataclass(frozen=True)
class TranscriptionPlanSegment:
    start: float
    end: float
    speaker_id: int
...
def resolve_output_path(source_path: Path, output_target: Path) -> Path:
    final_path = output_target if output_target.suffix.lower() == ".txt" else output_target / f"{source_path.stem}_transcript.txt"
...
def run_ffmpeg_normalize(source_path: Path, output_wav_path: Path) -> None:
    ...
    if completed.returncode != 0 or not output_wav_path.exists() or output_wav_path.stat().st_size == 0:
...
def audio_duration_seconds(waveform: torch.Tensor) -> float:
...
def ensure_speaker_model_cached() -> Path:
    ...
    downloaded = snapshot_download(SPEAKER_MODEL_ID, revision=SPEAKER_MODEL_REVISION, cache_dir=str(THREE_D_SPEAKER_CACHE))
    ...
    config = Config(
        {
            "feature_extractor": {
                ...
                "args": {"n_mels": 80, "sample_rate": SAMPLE_RATE, "mean_nor": True},
            },
            "embedding_model": {
...
def load_vad_model() -> AutoModel:
    ...
    return call_silently(AutoModel, model=str(VAD_MODEL), device="cpu", disable_update=True)


def load_asr_model() -> AutoModel:
    ensure_file(SENSEVOICE_MODEL, "local SenseVoice model directory")
    return call_silently(AutoModel, model=str(SENSEVOICE_MODEL), device="cpu", disable_update=True)
...
def run_vad(vad_model: AutoModel, wav_path: Path) -> list[list[float]]:
    ...
    return [[item[0] / 1000.0, item[1] / 1000.0] for item in result[0]["value"] if item[1] > item[0]]


def chunk_interval(
    start: float,
    end: float,
    duration: float = DIARIZATION_CHUNK_DURATION_SECONDS,
    step: float = DIARIZATION_CHUNK_STEP_SECONDS,
) -> list[list[float]]:
    if end <= start:
        return []
    ...
    while chunk_start < end:
        ...
        if chunk_end - chunk_start >= MIN_TRANSCRIPTION_SEGMENT_SECONDS:
...
def cluster_embeddings(embeddings: np.ndarray) -> np.ndarray:
    ...
    labels = ahc_cluster(embeddings) if embeddings.shape[0] < 40 else spectral_cluster(embeddings)
...
def normalize_diarization_segments(
    ...
    for segment in sorted(raw_segments, key=lambda item: (item.start, item.end)):
        ...
        if end <= start:
            ...
            continue
        if end - start < MIN_DIARIZATION_SEGMENT_SECONDS:
        ...
        if end <= start or end - start < MIN_DIARIZATION_SEGMENT_SECONDS:
...
def nearest_speaker_id(start: float, end: float, diarization_segments: list[DiarizationSegment]) -> int:
    center = (start + end) / 2
    return min(
...
def speaker_for_interval(start: float, end: float, diarization_segments: list[DiarizationSegment]) -> int:
    overlaps = [
        (overlap_seconds(start, end, segment.start, segment.end), segment.speaker_id)
        for segment in diarization_segments
    ]
    best_overlap, speaker_id = max(overlaps, key=lambda item: item[0])
    if best_overlap > 0:
        return speaker_id
    return nearest_speaker_id(start, end, diarization_segments)


def split_interval(start: float, end: float, max_duration: float) -> list[tuple[float, float]]:
    pieces: list[tuple[float, float]] = []
    current = start
    while current < end:
        piece_end = min(end, current + max_duration)
        if piece_end - current >= MIN_TRANSCRIPTION_SEGMENT_SECONDS:
...
) -> list[TranscriptionPlanSegment]:
    if total_duration <= 0:
        raise TranscriptionError("Normalized audio is empty")
    if not diarization_segments:
        raise DiarizationError("No diarization segments available for speaker assignment")

    boundaries = {0.0, total_duration}
    for segment in diarization_segments:
        boundaries.add(max(0.0, min(total_duration, segment.start)))
        boundaries.add(max(0.0, min(total_duration, segment.end)))

    plan: list[TranscriptionPlanSegment] = []
    sorted_boundaries = sorted(boundaries)
    for start, end in zip(sorted_boundaries, sorted_boundaries[1:]):
        if end - start < MIN_TRANSCRIPTION_SEGMENT_SECONDS:
            continue
        speaker_id = speaker_for_interval(start, end, diarization_segments)
        for piece_start, piece_end in split_interval(start, end, MAX_TRANSCRIPTION_SEGMENT_SECONDS):
            plan.append(TranscriptionPlanSegment(start=piece_start, end=piece_end, speaker_id=speaker_id))

    if not plan:
        speaker_id = nearest_speaker_id(0.0, total_duration, diarization_segments)
        plan = [
            TranscriptionPlanSegment(start=start, end=end, speaker_id=speaker_id)
            for start, end in split_interval(0.0, total_duration, MAX_TRANSCRIPTION_SEGMENT_SECONDS)
        ]
    return plan
...
def save_segment_audio(
    waveform: torch.Tensor,
    segment: TranscriptionPlanSegment,
    output_path: Path,
) -> None:
    start_frame = max(0, int(segment.start * SAMPLE_RATE))
    end_frame = min(waveform.shape[1], int(math.ceil(segment.end * SAMPLE_RATE)))
...
def transcribe_segments(
    transcription_plan: list[TranscriptionPlanSegment],
    waveform: torch.Tensor,
    asr_model: AutoModel,
    segment_dir: Path,
    ...
    for index, segment in enumerate(transcription_plan):
...
def build_result(
    ...
    return {
        ...
        "transcription_plan_count": transcription_plan_count,
...
def main() -> int:
    ...
    try:
        ...
        try:
            ...
            diarization_segments, normalization_failures, normalization_warnings = normalize_diarization_segments(raw_segments)
            ...
            transcription_plan = build_transcription_plan(diarization_segments, audio_duration_seconds(waveform))
            ...
            result = build_result(
                ...
                transcription_plan_count=len(transcription_plan),
